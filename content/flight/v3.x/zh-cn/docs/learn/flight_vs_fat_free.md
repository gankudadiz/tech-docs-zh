...
- Fat-Free 有一个称为 Mapper 的内置 ORM，可用于与数据库交互。Flight 有 [active-record](../../awesome-plugins/active-record)。
...
- Flight 有一个简单的 [PdoWrapper](../../pdo-wrapper)，比 Fat-Free 内置的 `DB\SQL` 类稍微更简单。
- Flight 有一个[权限插件](../../awesome-plugins/permissions)可用于保护应用安全。Fat-Free 需要使用第三方库。
- Flight 有一个称为 [active-record](../../awesome-plugins/active-record) 的 ORM，比 Fat-Free 的 Mapper 更像 ORM。`active-record` 的额外好处是你可以定义记录间的关系以实现自动连接，而 Fat-Free 的 Mapper 需要你创建 [SQL 视图](https://fatfreeframework.com/3.8/databases#ProsandCons)。
