...
[`in()`](validation#in) 规则确保用户无法选择不在选项列表中的选项。这是数据完整性的重要规则，因此 Filament 默认将其应用于所有选择器字段。

:::warning
选中选项验证至关重要，因此我们强烈建议你为表单[编写自动化测试](../testing/testing-schemas#testing-form-validation)以确保验证按预期工作。
:::
