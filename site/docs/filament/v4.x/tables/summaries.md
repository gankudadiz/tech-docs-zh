...
默认情况下，会有一行当前页面数据的汇总行，如果有多页数据，还会有一行所有数据的总计汇总行。你还可以为记录的[分组](grouping)添加汇总，请参阅["汇总行组"](#summarising-groups-of-rows)。
...
你还可以[创建自己的自定义汇总器](#custom-summaries)以任何你希望的方式显示数据。
...
计数可用于查找数据集中值的总数。除非你只想计算行数，否则你可能还需要[限定数据集范围](#scoping-the-dataset)：
...
此功能对于[计数](#count)汇总器特别有用，因为它可以计算数据集中有多少记录通过了测试：
...
当你使用[分组汇总](#summarising-groups-of-rows)且只想显示每组的汇总，或者页面汇总是多余的而你只需要所有记录的总计时，这很有用。
